...
|---|---|---|
| `/` Home | Convert to first run | Hero tagline; **asciinema demo** of a real migration (the §4 GTM "show moment"); the 4-step flow diagram (init → plan → check → migrate → cutover); install block (script + brew + direct download tabs); "how it stays safe" trio (dry-run plan, idempotent reruns, maintenance-mode with crash-safe cleanup); framework support table (Drupal, WordPress, static now; Laravel/Joomla soon); GitHub stars badge; comparison teaser table from MARKETING.md §3 |
