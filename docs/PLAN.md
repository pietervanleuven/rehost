...
- Init Go module, repo layout (`cmd/`, `internal/detect`, `internal/tui`, `internal/project`, `internal/check`, `internal/recipe`, `internal/inventory`, `internal/state`; the generic SSH, DB, transfer, DNS and search-replace layers were extracted into standalone `go-*` modules 2026-08-29 and folded back into `internal/` 2026-09-15 — kept generic for cheap re-extraction)
