...
// Probe detects the host's capabilities over any Runner: shell, uname, home,
// PHP version and tool availability. It prefers a single compound POSIX
// script (one round trip — shared hosts are slow and may cap sessions) and
// degrades to one command per session for restricted shells. host is only
// used to label the result; Capabilities.User is left for the caller, who
// knows the connection's identity.
func Probe(ctx context.Context, r Runner, host string) (*Capabilities, error) {
