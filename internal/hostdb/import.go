...
type ImportOptions struct {
	...
	// utf8mb4 (the safe modern default); callers that
...
// file on disk:
//
//   - A mode-600 FIFO is created under $HOME/StageDir. A FIFO is an inode with no
...
func importCreds(ctx context.Context, conn Conn, creds *Credentials, charset string, remoteGunzip bool, payload io.Reader) error {
	...
	fifoRef := `"$HOME"/` + StageDir + `/.import-creds.` + token

	setup := `mkdir -p "$HOME"/` + StageDir + ` && mkfifo -m 600 ` + fifoRef
