...
// in a mode-600 file under $HOME/StageDir for the seconds the command runs —
...
func pgWithPassFile(creds *Credentials, cmd string) (string, error) {
	...
	marker := "HOSTDB_PGPASS"
	for strings.Contains(line, marker) {
		marker += "Z"
	}
	return `umask 077 && mkdir -p "$HOME"/` + StageDir + ` && t="$HOME"/` + StageDir + `/.pgpass.` + token +
...
func importPGRun(ctx context.Context, conn Conn, creds *Credentials, remoteGunzip bool, payload io.Reader) error {
	...
	passRef := `"$HOME"/` + StageDir + `/.pgpass.` + token
	marker := "HOSTDB_PGPASS"
	for strings.Contains(line, marker) {
		marker += "Z"
	}
	setup := `umask 077 && mkdir -p "$HOME"/` + StageDir + ` && cat > ` + passRef + ` <<'` + marker + `'` +
		"\n" + line + "\n" + marker
