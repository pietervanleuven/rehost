...
if (!is_array($cfg) || !isset($cfg['name']) || $cfg['name'] === '') {
  fwrite(STDERR, "hostdb: no database config on stdin\n");
  exit(1);
}
if (!function_exists('gzopen')) {
  fwrite(STDERR, "hostdb: the PHP zlib extension is missing\n");
  exit(1);
}
...
try {
  ...
  try {
    ...
    $views = array();
    $note('hostdb: skipped views: ' . $e->getMessage());
  }
  foreach ($views as $row) {
    ...
    try {
      ...
      $note('hostdb: skipped view ' . $view . ': ' . $e->getMessage());
      continue;
    }
    if (!isset($create[0][1]) || $create[0][1] === null || $create[0][1] === '') {
      $note('hostdb: skipped view ' . $view . ': SHOW CREATE VIEW returned no definition (insufficient privileges?)');
      continue;
    }
  ...
  try {
    ...
    $triggers = array();
    $note('hostdb: skipped triggers: ' . $e->getMessage());
  }
  foreach ($triggers as $row) {
    ...
    try {
      ...
      $note('hostdb: skipped trigger ' . $trigger . ': ' . $e->getMessage());
      continue;
    }
    if (!isset($create[0][2]) || $create[0][2] === null || $create[0][2] === '') {
      $note('hostdb: skipped trigger ' . $trigger . ': SHOW CREATE TRIGGER returned no definition (insufficient privileges?)');
      continue;
    }
  ...
  foreach (array('PROCEDURE', 'FUNCTION') as $kind) {
    try {
      ...
      $note('hostdb: skipped ' . $kind . 's: ' . $e->getMessage());
      continue;
    }
    foreach ($routines as $row) {
      ...
      try {
        ...
        $note('hostdb: skipped ' . $kind . ' ' . $name . ': ' . $e->getMessage());
        continue;
      }
      if (!isset($create[0][2]) || $create[0][2] === null || $create[0][2] === '') {
        $note('hostdb: skipped ' . $kind . ' ' . $name . ': SHOW CREATE ' . $kind . ' returned no definition (insufficient privileges?)');
        continue;
      }
  ...
  fwrite(STDERR, 'hostdb: dump failed: ' . $e->getMessage() . "\n");
  exit(1);
}
`
