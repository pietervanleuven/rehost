...
func TestDumpPHPRemoteFailure(t *testing.T) {
	s := &fakeStreamer{res: remote.Result{ExitCode: 1, Stderr: "hostdb: dump failed: connect failed for hunter2\n"}}
...
// looks its SQL up in the JSON dataset at $HOSTDB_FAKE_DB: a matching entry
...
$GLOBALS['__hostdb_db'] = json_decode(file_get_contents(getenv('HOSTDB_FAKE_DB')), true);
$GLOBALS['__hostdb_err'] = '';
...
function mysqli_query($db, $sql, $mode = 0) {
  $q = isset($GLOBALS['__hostdb_db']['queries'][$sql]) ? $GLOBALS['__hostdb_db']['queries'][$sql] : null;
  if ($q === null) { $GLOBALS['__hostdb_err'] = ''; return new RehostFakeResult(array()); }
  if (isset($q['error']) && $q['error'] !== '') { $GLOBALS['__hostdb_err'] = $q['error']; return false; }
  $GLOBALS['__hostdb_err'] = '';
  return new RehostFakeResult(isset($q['rows']) ? $q['rows'] : array());
}
function mysqli_error($db) { return $GLOBALS['__hostdb_err']; }
...
func runPHPDumpFake(t *testing.T, dataset map[string]any) string {
	...
	cmd.Env = append(os.Environ(), "HOSTDB_FAKE_DB="+dataPath)
...
func TestPHPDumpEmitsViewsTriggersRoutines(t *testing.T) {
	...
	skip := mustContain("-- hostdb: skipped PROCEDURE p1:")
...
func TestPHPDumpTriggerQueryDenied(t *testing.T) {
	...
	sql := runPHPDumpFake(t, dataset)
	if !strings.Contains(sql, "-- hostdb: skipped triggers:") {
...
func TestPHPDumpCommandThroughShell(t *testing.T) {
	...
	if !strings.Contains(stderr.String(), "hostdb: no database config") {
